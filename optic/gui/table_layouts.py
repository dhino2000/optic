...
def makeLayoutROIFilterThreshold(widget_manager, key_label, key_lineedit, dict_roi_threshold, num_row=2, num_col=3):
    ...
    for i, (key_threshold, value_threshold) in enumerate(dict_roi_threshold.items()):
        row = i // num_col
        col = i % num_col
        layout.addLayout(makeLayoutLineEditLabel(widget_manager, key_label, key_lineedit, label=key_threshold, axis="vertical", text_set=value_threshold, width_fix=None), row, col, 1, 1)
